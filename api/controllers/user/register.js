    ...
    // Data collect and insert into Database
      ...
      if(userFind.length > 0){
        this.res.status(responseCode.ServerErrors.Codes.USER_ALREADY_EXIST.error_code).send(responseCode.ServerErrors.Codes.USER_ALREADY_EXIST)
          ...
          let user = await User.create({ username : inputs.username, password : encryptPassword , full_name : inputs.full_name , country_code : inputs.country_code, 
          ...
          if(user){
            ...
            await sails.helpers.sendNotification.with({
              ...
              type: inputs.registrationType == "PAID" ? "new_request_paid" : "new_request_free"
            });
